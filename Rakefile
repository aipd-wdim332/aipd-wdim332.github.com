...
# Usage: rake profile name="John Smith" [date="2012-02-09"]
desc "Begin a new profile in #{CONFIG['posts']}"
task :profile do
  abort("rake aborted: '#{CONFIG['posts']}' directory not found.") unless FileTest.directory?(CONFIG['posts'])
  title = ENV["name"] || "new-profile"
  slug = title.downcase.strip.gsub(' ', '-').gsub(/[^\w-]/, '')
  begin
    date = (ENV['date'] ? Time.parse(ENV['date']) : Time.now).strftime('%Y-%m-%d')
  rescue Exception => e
  ...
  puts "Creating new profile: #{filename}"
  open(filename, 'w') do |post|
    post.puts "---"
    post.puts "layout: profile"
    post.puts "title: \"#{title.gsub(/-/,' ')}\""
    post.puts 'description: ""'
    post.puts "category: profile"
    post.puts "tags: []"
    post.puts "---"
    post.puts "{% include JB/setup %}"
  end
